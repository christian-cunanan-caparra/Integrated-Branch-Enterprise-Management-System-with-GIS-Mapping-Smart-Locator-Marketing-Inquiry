...
<head>
    ...
    <title><?= $title ?? 'Inquiries Management' ?></title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .stat-card:nth-child(2)::before { background: linear-gradient(90deg, var(--warning), #f1c40f); }
        ...
        .badge-status.contacted .dot { background: #007bff; }
        .badge-status.resolved .dot { background: #28a745; }
        ...
        .status-select.pending { color: #856404; background: #fff3cd; }
        .status-select.contacted { color: #004085; background: #cce5ff; }
        .status-select.resolved { color: #155724; background: #d4edda; }
        ...
        <div class="row g-0">
            ...
            <div class="col-md-2 sidebar">
                ...
                <div class="sidebar-nav">
                    ...
                    <a href="<?= base_url('admin/inquiries') ?>" class="active">
            ...
            <div class="col-md-10 main-content">
                ...
                <div class="top-bar d-flex justify-content-between align-items-center flex-wrap">
                    <h4><i class="bi bi-chat-dots"></i> Inquiries Management</h4>
                    <div class="user-info">
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-3 col-6 mb-2">
                        <div class="stat-card" onclick="window.location.href='<?= base_url('admin/inquiries?status=pending') ?>'">
                    ...
                    <div class="col-md-3 col-6 mb-2">
                        <div class="stat-card" onclick="window.location.href='<?= base_url('admin/inquiries?status=contacted') ?>'">
                            <div class="stat-icon"><i class="bi bi-telephone-fill"></i></div>
                            <div class="stat-number" style="color: var(--info);"><?= $statusCounts['contacted'] ?? 0 ?></div>
                            <div class="stat-label">Contacted</div>
                        </div>
                    </div>
                    <div class="col-md-3 col-6 mb-2">
                        <div class="stat-card" onclick="window.location.href='<?= base_url('admin/inquiries?status=resolved') ?>'">
                            <div class="stat-icon"><i class="bi bi-check-circle-fill"></i></div>
                            <div class="stat-number" style="color: var(--success);"><?= $statusCounts['resolved'] ?? 0 ?></div>
                ...
                <div class="row mb-4">
                        ...
                        <div class="search-box">
                            <div class="input-group">
                                <input type="text" class="form-control" id="searchInput" 
                                       placeholder="Search by name, city, province, or branch..." 
                    ...
                    <div class="col-md-4">
                        <select class="filter-select" id="statusFilter" onchange="window.location.href='<?= base_url('admin/inquiries') ?>?status='+this.value">
                                ...
                                <?php if (!empty($inquiries)): ?>
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td>
                                                <div class="inquiry-name"><?= esc($inquiry['full_name'] ?? 'N/A') ?></div>
                                                <?php if (!empty($inquiry['email'])): ?>
                                                    <small class="text-muted"><i class="bi bi-envelope"></i> <?= esc($inquiry['email']) ?></small>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <div class="inquiry-location">
                                                    <div><i class="bi bi-geo-alt"></i> <?= esc($inquiry['city'] ?? 'N/A') ?></div>
                                                    <div><i class="bi bi-tag"></i> <?= esc($inquiry['province'] ?? 'N/A') ?></div>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="inquiry-contact">
                                                    <i class="bi bi-phone"></i> <?= esc($inquiry['contact_number'] ?? 'N/A') ?>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="inquiry-branch">
                                                    <i class="bi bi-geo-alt-fill"></i>
                                                    <?= esc($inquiry['suggested_branch_name'] ?? 'Not assigned') ?>
                                                </div>
                                            </td>
                                            <td>
                                                <select class="status-select <?= $inquiry['status'] ?? 'pending' ?>" 
                                                        onchange="updateStatus(<?= $inquiry['id'] ?>, this.value)">
                                                    <option value="pending" <?= ($inquiry['status'] ?? 'pending') == 'pending' ? 'selected' : '' ?>>Pending</option>
                                            ...
                                            <td>
                                                <small class="text-muted">
                                                    <?= !empty($inquiry['created_at']) ? date('M d, Y', strtotime($inquiry['created_at'])) : 'N/A' ?>
                                                </small>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                            ...
                            <div class="row align-items-center">
                                <div class="col-md-6">
                                    <div class="pagination-info">
                                        Showing <strong><?= count($inquiries ?? []) ?></strong> of <strong><?= $totalInquiries ?? 0 ?></strong> inquiries
                                        <?php if (!empty($search)): ?>
                                    ...
                                    <nav aria-label="Page navigation">
                                        <ul class="pagination justify-content-end">
                                            <li class="page-item <?= ($currentPage ?? 1) <= 1 ? 'disabled' : '' ?>">
                                                <a class="page-link" href="<?= base_url('admin/inquiries?page=' . (($currentPage ?? 1) - 1) . (!empty($search) ? '&search=' . urlencode($search) : '') . (!empty($statusFilter) && $statusFilter !== 'all' ? '&status=' . urlencode($statusFilter) : '')) ?>">
                                            ...
                                            if ($startPage > 1): ?>
                                                <li class="page-item">
                                                    <a class="page-link" href="<?= base_url('admin/inquiries?page=1' . (!empty($search) ? '&search=' . urlencode($search) : '') . (!empty($statusFilter) && $statusFilter !== 'all' ? '&status=' . urlencode($statusFilter) : '')) ?>">1</a>
                                                </li>
                                                <?php if ($startPage > 2): ?>
                                            ...
                                            <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                                                <li class="page-item <?= ($i == ($currentPage ?? 1)) ? 'active' : '' ?>">
                                                    <a class="page-link" href="<?= base_url('admin/inquiries?page=' . $i . (!empty($search) ? '&search=' . urlencode($search) : '') . (!empty($statusFilter) && $statusFilter !== 'all' ? '&status=' . urlencode($statusFilter) : '')) ?>"><?= $i ?></a>
                                                </li>
                                            <?php endfor; ?>
                                            <?php if ($endPage < ($totalPages ?? 1)): ?>
                                                ...
                                                <?php endif; ?>
                                                <li class="page-item">
                                                    <a class="page-link" href="<?= base_url('admin/inquiries?page=' . ($totalPages ?? 1) . (!empty($search) ? '&search=' . urlencode($search) : '') . (!empty($statusFilter) && $statusFilter !== 'all' ? '&status=' . urlencode($statusFilter) : '')) ?>"><?= $totalPages ?? 1 ?></a>
                                                </li>
                                            <?php endif; ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function performSearch() {
                ...
                let url = '<?= base_url('admin/inquiries') ?>?';
        ...
        function updateStatus(id, status) {
            // Show loading state
            const select = document.querySelector(`.status-select[onchange*="${id}"]`);
            if (select) {
                select.disabled = true;
                select.style.opacity = '0.6';
            }

            fetch('<?= base_url('inquiry/updateStatus') ?>/' + id, {
                method: 'POST',
                headers: {
